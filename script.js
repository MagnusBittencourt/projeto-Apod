...
function apiApod() {
  ...
  $.ajax({
    url: `https://api.nasa.gov/planetary/apod?api_key=Q5e8bCBnZzizYnOZjBRpceYwZZuNFsFq9AxfCu3b&date=${data}`,
...
function apiData(saida) {
  ...
  if (saida.media_type == "image") {
    ...
    imagem.html(
      `<iframe class="img" src="${saida.url}?autoplay=1&mute=1"></iframe>`
    );
  }
}
